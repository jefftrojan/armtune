...
def render_html_report(
    ...
    download_links = [
        ("report.md", "Markdown report"), ("results.csv", "Results (CSV)"),
        ("results_raw.json", "Raw llama-bench output (JSON)"), ("recommended_launch.sh", "Launch script"),
    ]
    if concurrency_results:
        download_links.append(("concurrency_raw.json", "Concurrency results (JSON)"))
    downloads_html = " &middot; ".join(f'<a href="{href}" download>{label}</a>' for href, label in download_links)
