...
echo "  2. Run the sweep, pointing at any Hugging Face model:"
echo "       # a repo that already publishes a GGUF file:"
echo "       armtune sweep --hf-repo <org>/<model>-GGUF --hf-file <model>-f16.gguf \\"
echo "         --llama-bin-dir llama.cpp/build/bin"
echo "       # OR a plain (safetensors) repo -- armtune converts it via"
echo "       # llama.cpp's convert_hf_to_gguf.py (needs its Python deps:"
echo "       # pip install -r llama.cpp/requirements.txt):"
echo "       armtune sweep --hf-repo <org>/<model> --llama-bin-dir llama.cpp/build/bin"
echo "       # OR a GGUF file you already have locally:"
echo "       armtune sweep --base-model models/<model>-f16.gguf --llama-bin-dir llama.cpp/build/bin"
echo "  3. Check results/report.md for the winning config."
